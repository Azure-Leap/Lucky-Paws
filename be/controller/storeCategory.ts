...
const getStoreCategory = async (req: Request, res: Response) => {
  ...
  const deleteStoreCategory = async (req:Request, res:Response) => {
    ...
    try {
      ...
      res.status(201).json({ message: `${id} - tai amitanii medeelel ustlaa`, storeCategory });
    } catch (error) {
        console.log("ERROR", error);
    }
  };
...
export { getStoreCategory,getAllStoreCategories,deleteStoreCategory,updateStoreCategory,createStoreCategory };