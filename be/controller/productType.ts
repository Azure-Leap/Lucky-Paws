...
const getProductType = async (req: Request, res: Response) => {
  ...
  const deleteProductType = async (req:Request, res:Response) => {
    ...
    try {
      ...
      res.status(201).json({ message: `${id} - tai amitanii medeelel ustlaa`, productType });
    } catch (error) {
        console.log("ERROR", error);
    }
  };
...
export { getProductType,getAllProductTypes,deleteProductType,updateProductType,createProductType };